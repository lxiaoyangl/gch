          ...
          <el-tab-pane label="组件部署" name="componentDeployment">
            ...
            >
              ...
              <el-form-item prop="classNameType" label="类型">
                <el-select
                  v-model="formData.classNameType"
                  placeholder="请选择类路径类型"
                >
                  <el-option
                    v-for="(item, i) in classNameTypeOpts"
                    :key="i"
                    :label="item"
                  ></el-option>
                </el-select>
              ...
              <el-form-item prop="className" label="类路径">
                <el-select
                  ...
                  placeholder="请选择javaClass路径"
                >
                  <el-option
                    v-for="(item, i) in classNameOpts"
                    :key="i"
                    :label="item"
                  ></el-option>
                </el-select>
              ...
              <el-form-item prop="extention" label="变参集合">
                <el-input
                  v-model="formData.extention"
                  placeholder="请输入变参key集合"
                ></el-input>
              </el-form-item>
              <el-form-item prop="dtaModel" label="数据模型">
                <el-select
                  v-model="formData.dtaModel"
                  placeholder="请选择数据模型"
                >
                  <el-option
                    v-for="(item, i) in dtaModelOpts"
                    :key="i"
                    :label="item"
                  ></el-option>
                </el-select>
...
export default {
  ...
  methods: {
    async getPerformanceList() {
      let { code, data } = await getPerformanceList();
      if (code == 200) {
      }
    },
    handleSave() {
      this.$refs.formData.validate((valid) => {
        if (valid) {
          let { code, data } = createPerformanceNode(this.formData);
          if (code == 200) {
          }
        } else {
