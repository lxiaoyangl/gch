          ...
          <div class="application" :class="applicationBackGorundClass(item)" @mouseover="applicationClassMouseOver($event,item)" @mouseout="applicationClassMouseLeave($event,item)">
            <div class="applicationDesc">
              ...
              <div style="position: absolute;right: 20px;">
                <div v-if="item.status.indexOf('1-4') != 0" class="applicationDescButton" @click="handleAdd">查看详情</div>
                <div v-if="item.status.indexOf('1-4') != 0 && item.status.indexOf('1-') >= 0" class="applicationDevButton" @click="handleAdd">开发更新</div>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.$router.push({path: '/fidlddata'})
    },
    handleEdit(item) {
      let routeData = this.$router.resolve({
